...
const tags = document.getElementById('problemTags');
...
window.onbeforeunload = fetchUrl;
window.onload = fetchUrl;

function fetchUrl(fromDate = true) {
    fetch(apiUrl)
    ...
    .then(data => {
        ...
        tags.innerHTML = "Tags: " + alltags.slice(0, alltags.length-2);
    })
    .catch(error => {
